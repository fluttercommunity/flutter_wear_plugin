...
class Wear {
  ...
  Future<dynamic> _onMethodCallHandler(MethodCall call) async {
    switch (call.method) {
      case 'onEnterAmbient':
        ...
        final details = AmbientDetails(args['burnInProtection'] ?? false, args['lowBitAmbient'] ?? false);
  ...
  Future<String> getShape() async {
    try {
      final result = await _channel.invokeMethod<String>('getShape');
      return result ?? 'round';
  ...
  Future<bool> isAmbient() async {
    try {
      final result = await _channel.invokeMethod<bool>('isAmbient');
      return result ?? false;
    } on PlatformException catch (e, st) {
